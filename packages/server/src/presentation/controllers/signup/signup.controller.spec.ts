...
describe('SignUpController', () => {
  test('should throw error case has no email field', async () => {
    ...
    await expect(
      async () => await signupController.handle(fakeUserRegister, fakeContextGraphQL)
  ...
  test('should create an user case everything is ok', async () => {
    ...
    jest
      ...
      .mockResolvedValueOnce(await new Promise((resolve, reject) => resolve(resolvedValue)))
    ...
    expect(await signupController.handle(fakeUserRegister, fakeContextGraphQL)).toStrictEqual(
      expectedResponse
    )
  })
})
